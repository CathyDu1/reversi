...
/* If we are not on Heroku, then we need to adjest our port and directory*/
if((typeof port == 'undefined') || (port === null)){
    port = 8080;
    directory = './public';
}

/*Set up our static file web server to deliver files form the file system*/
...
io.on('connection',(socket) => {
    ...
    /* expected payload passes from the client:
        {
    ...
    socket.on('join_room', (payload) => {
        ...
        io.in(room).fetchSockets().then((sockets) =>{
            ...
            if((typeof sockets == 'undefined') || (sockets === null) || !sockets.includes(socket)){
                ...
                socket.emit('join_room_response',response);
                serverLog('join_room command failed',JSON.stringify(reponse));
            }
    ...
    socket.on('invite', (payload) => {
        ...
        if((typeof room == 'undefined') || (room === null)||(room === "")){
            response = {
                result: 'fail',
                message: 'the user that was invited is not have a room'
        ...
        io.in(room).allSockets().then((sockets) =>{
            ...
            /* Invitee is in the room */
                ...
                serverLog('invite command successed', JSON.stringify(response));
            }
        });
    });
    ...
    socket.on('uninvite', (payload) => {
        ...
        if((typeof room == 'undefined') || (room === null)||(room === "")){
            response = {
                result: 'fail',
                message: 'the user that was uninvited is not have a room'
        ...
        io.in(room).allSockets().then((sockets) =>{
            ...
            /* Uninvitee is in the room */
                ...
                serverLog('uninvite command successed', JSON.stringify(response));
            }
        });
    });
    ...
    socket.on('game_start', (payload) => {
        ...
        /*Make sure that the to engage player is present*/
        io.in(room).allSockets().then((sockets) =>{
            ...
            /* Engaged player  is in the room */
                ...
                serverLog('game_start command successed', JSON.stringify(response));
            }
        });
    });
    ...
    socket.on('send_chat_message', (payload) => {
        ...
        if((typeof room == 'undefined') || (room === null)){
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a valid message to room';
            socket.emit('send_chat_message_response',response);
            serverLog('send_chat_message command failed', JSON.stringify(response));
            return;
        }
        ...
        serverLog('send_chat_message command successed', JSON.stringify(response));
    ...
    socket.on('play_token', (payload) => {
        ...
        /* If it is white's turn, but the socket playing is not white player, then fail */
...
/* Check r+dr+dr and c+dc+dc is the same color with the current player's color */
function check_line_match(color,dr,dc,r,c,board){
    ...
    /* Check to make sure we did not walk off the board */
    if((r+dr<0)||(r+dr>7)){
        return false;
    }
    if((c+dc<0)||(c+dc>7)){
...
/* Return true if r+ds is legal move and c+dc is legal move */
...
function flip_line(who,dr,dc,r,c,board){
    ...
    //base case, return when we reach to the other end of the line
...
function send_game_update(socket,game_id,message){
    ...
    /* Check to see is a game with game_id exists */
